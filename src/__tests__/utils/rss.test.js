...
describe("RSS Utils", () => {
  beforeEach(() => {
    ...
    // Reset return values
    mockAlertHubUtils.generateURLForTheFeed.mockReturnValue(
  ...
  describe("createRSSFeed", () => {
    ...
    test("should use constructor config when no parameter provided", async () => {
      mockRssClient.processFeed.mockImplementation(
        (_feedName, _format, callback) => {
    ...
    test("should include extra feeds in sources", async () => {
      mockRssClient.processFeed.mockImplementation(
        (_feedName, _format, callback) => {
          ...
          expect(
            sources.some(
              (source) => source.feed_url === "https://external-feed.com/rss",
            ),
          ).toBe(true);
    ...
    test("should process feed and return result", async () => {
      ...
      expect(mockRssClient.processFeed).toHaveBeenCalledWith(
        "alertHub",
        "rss",
        expect.any(Function),
      );
    ...
    test("should reject promise when RSS processing fails", async () => {
      ...
      await expect(rssUtils.createRSSFeed()).rejects.toThrow(

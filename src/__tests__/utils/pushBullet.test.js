...
describe("PushBullet Utils", () => {
  beforeEach(() => {
    ...
    mockPusher.link.mockImplementation(
      (_linkData, _title, _url, _body, callback) => {
  ...
  describe("sendPushBulletNotification", () => {
    ...
    test("should create PushBullet instance with access token", async () => {
      await pushBulletUtils.default.sendPushBulletNotification(
        mockConfig,
        mockFeedData,
      );
    ...
    test("should return false when push notification fails", async () => {
      ...
      mockPusher.link.mockImplementation(
        (_options, _title, _link, _body, callback) => {
          callback(error, null);
        },
      );

      const result = await pushBulletUtils.default.sendPushBulletNotification(
        mockConfig,
        mockFeedData,
      );

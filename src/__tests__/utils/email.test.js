...
describe('Email Utils', () => {
  ...
  describe('sendEmailNotification', () => {
    ...
    test('should create transporter with provided config', async () => {
      ...
      expect(mockNodemailer.createTransport).toHaveBeenCalledWith(mockConfig.config);
    });
    ...
    test('should send email with correct options including subject prefix', async () => {
      ...
      expect(mockTransporter.sendMail).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          text: expect.stringContaining('https://github.com/owner/repo/releases/tag/v1.0.0'),
          html: expect.stringContaining('<a target="_blank" href="https://github.com/owner/repo/releases/tag/v1.0.0">New Release v1.0.0</a>')
        }),
        expect.any(Function)
      );
    });
    ...
    test('should send email without subject prefix when prefix is empty', async () => {
      ...
      await emailUtils.default.sendEmailNotification(configWithoutPrefix, mockFeedData);
    ...
    test('should include HTML version with original formatting', async () => {
      ...
      expect(mockTransporter.sendMail).toHaveBeenCalledWith(
        expect.objectContaining({
          html: expect.stringContaining('<p>This is a <strong>test</strong> release</p>')
        }),
        expect.any(Function)
      );
    });
    ...
    test('should handle email sending errors gracefully', async () => {
      ...
      const result = await emailUtils.default.sendEmailNotification(mockConfig, mockFeedData);
